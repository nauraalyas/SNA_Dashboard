...
function setupFilterControls() {
  const chipList = document.getElementById("facultyChipList");
  chipList.innerHTML = allFacultyInitials().map(init => {
    const fac = [...STATE.facultyById.values()].find(f => f.faculty_initials === init);
    return `<button type="button" class="faculty-chip" data-fac="${init}" title="${fac ? fac.faculty_name : ""}">${init}</button>`;
  }).join("");
  chipList.querySelectorAll(".faculty-chip").forEach(btn => {
    btn.addEventListener("click", () => {
      const fac = btn.dataset.fac;
      if (STATE.facultyFilter.has(fac)) {
        STATE.facultyFilter.delete(fac);
        btn.classList.remove("active");
      } else {
        STATE.facultyFilter.add(fac);
  ...
  document.getElementById("resetFiltersBtn").addEventListener("click", () => {
    ...
    STATE.minWeight = 1;
    STATE.searchTerm = "";
    chipList.querySelectorAll(".faculty-chip.active").forEach(b => b.classList.remove("active"));
